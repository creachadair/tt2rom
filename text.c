...
{
  if(line == NULL)
    return 1;  /* we will consider a NULL pointer a blank line */
...
int read_line(FILE *ifp, char *buf, int len)
{
  ...
  if(fgets(buf, len, ifp) == NULL)
    return 0; /* no more lines available */
